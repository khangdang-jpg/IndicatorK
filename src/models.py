...
class Recommendation:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "action": self.action,
            "buy_zone_low": round(float(self.buy_zone_low), 2),
            "buy_zone_high": round(float(self.buy_zone_high), 2),
            "stop_loss": round(float(self.stop_loss), 2),
            "take_profit": round(float(self.take_profit), 2),
            "position_target_pct": round(float(self.position_target_pct), 4),
            "rationale_bullets": self.rationale_bullets,
            "entry_type": self.entry_type,
            "breakout_level": round(float(self.breakout_level), 2),
